...
# openai.api_key = "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"
...
#     chatgpt_output = f'{name}: {chatgpt_raw_output}'
#     chat_history += chatgpt_output + '\n'
#     history_file = os.path.join(threads_dir, f'{thread_id}.txt')
#     with open(history_file, 'a') as f:
#         f.write('\n'+ current_day+ ' '+ current_time+ ' User: ' +user_input +' \n' + current_day+ ' ' + current_time+  ' ' +  chatgpt_output + '\n')
#         f.close()
...
# # Function to get a response from the chatbot
# def get_response(userText, thread_id):
#     return chat(userText, thread_id)
...
# def get_history_file(thread_id):
#     messages = []
#     file_path = os.path.join(threads_dir, f'{thread_id}.txt')
#     if not os.path.exists(file_path):
#         return jsonify({"error": "File not found"}), 404
#     with open(file_path, 'r') as f:
#         lines = f.readlines()
...
# def delete_history_file(thread_id):
#     file_path = os.path.join(threads_dir, f'{thread_id}.txt')
#     if os.path.exists(file_path):
#         os.remove(file_path)
#     return '', 204
...
#         f.write('\n')
#     chat_history = ''
#     return jsonify({"message": "New thread created", "thread_id": thread_id})
...
#     return jsonify({"response": get_response(userText, thread_id)})
